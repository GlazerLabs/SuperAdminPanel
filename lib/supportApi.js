...
export function mapInboxRowToUi(raw, categoryFallback = "general") {
  ...
  const profilePicUrl =
    ...
    raw.avatar_url ??
    "";
  const lastUpdatedRaw = raw.last_message_at ?? raw.updated_at ?? raw.updatedAt ?? raw.last_updated_at ?? null;
  const lastUpdated = lastUpdatedRaw ? formatShortDate(String(lastUpdatedRaw)) : "—";
...
/**
 * GET /support/admin/tickets?conversation_id=&user_id=&tournament_id=&category=&page=&limit=
 */
export async function fetchSupportTickets(params) {
  const { conversation_id, user_id, tournament_id, category, page = 1, limit = 20 } = params;
  const query = {
  ...
  const total = pickTotal(json) || tickets.length;
  return { tickets, total, raw: json };
}
