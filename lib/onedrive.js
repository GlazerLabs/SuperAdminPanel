...
export async function uploadFileToLeadFolder(leadId, leadName, subfolder, fileName, fileBuffer, mimeType) {
  ...
  const targetSubfolder = DEFAULT_SUBFOLDERS.includes(subfolder) ? subfolder : "Others";
  ...
  let fileWebUrl = typeof uploaded?.webUrl === "string" ? uploaded.webUrl.trim() || null : null;
  ...
  if (!itemId) {
    try {
      const byPath = await client
        ...
        .get();
      itemId = byPath?.id;
      if (!fileWebUrl && typeof byPath?.webUrl === "string") fileWebUrl = byPath.webUrl.trim() || null;
    } catch {
      // ignore
    }
  }
  ...
  if (itemId) {
    if (!fileWebUrl) {
      try {
        ...
        const w = meta?.webUrl;
        if (typeof w === "string" && w.trim()) fileWebUrl = w.trim();
      } catch {
        // ignore
      }
    }
    if (!fileWebUrl) {
      try {
        ...
        const w = linkResult?.link?.webUrl;
        if (typeof w === "string" && w.trim()) fileWebUrl = w.trim();
      } catch {
        // ignore — caller may still have manual URL
      }
    }
  }
