...
import TournamentAcquisitionPanel from "@/components/Tournaments/TournamentAcquisitionPanel";
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="inline-flex items-center gap-1 rounded-full bg-slate-100 p-1 ring-1 ring-slate-200/80">
              {TABS.map((tab) => (
                <button
                  key={tab.id}
                  type="button"
          ...
          <p className="mt-1 text-slate-600">
            {activeTab === "hierarchy"
              ? "Organizer hierarchy tree"
              : "Users and teams acquired for this tournament"}
          </p>
        </div>
      </div>
      ...
      {activeTab === "hierarchy" ? (
        ...
        <TournamentAcquisitionPanel tournamentId={id} />
      )}
    </main>
  );
}
